...
metadata:
  hermes:
    ...
    related_skills: [kart-racer/kr-app-expert, kart-racer/mobile-app-expert]
---
...
3. **Horizon filter** (need ≥2 to call it a step forward): capability a small team can use in ≤90 days; cost/latency that changes a bet; interoperability (MCP, copilot policy, GHE); new risk/compliance duty; durability; **local relevance** to KartRacer, Cursor, GitHub/GHE, or defensive security.
...
|--------|------------------------|
| OpenAI + Azure OpenAI / ChatGPT Enterprise | KartRacer Coach/Ask; FedRAMP 20x Moderate path |
...
|---------|-------------------------|
| **KartRacer** | Coach/Ask (`OPENAI_MODEL`, default `gpt-4o-mini` in `ENVIRONMENT.md`), Render API, Vercel web, EAS iOS/Android. A model change is beneficial only if quality, cost, or data-use terms improve **without** new retention/training risk. |
| **Cursor** | Coding agents, MCP, Privacy Mode, Enterprise CVP for defensive cyber. |
| **GitHub / GHE** | `CG-67-R1/KartRacer` is GitHub.com today. GHE Server / Copilot Enterprise policies matter if the org turns Copilot on or moves to GHE. |
...
Outcomes (only if relevant to KartRacer, Cursor/GitHub/GHE, or cyber):
- …
