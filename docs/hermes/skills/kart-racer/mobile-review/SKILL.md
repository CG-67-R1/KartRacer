---
name: mobile-review
description: "KartRacer screen-by-screen mobile audit. Use with kart-racer/kr-app-expert for weekly reviews."
version: 1.1.0
...
metadata:
  hermes:
    ...
    related_skills: [kart-racer/kr-app-expert, kart-racer/track-data-analyst, requesting-code-review]
---

# KartRacer Mobile Developer Review

Screen-by-screen **mobile developer / QA review** for the KartRacer Expo app. **Report only** — do not commit, push, or edit source files unless the user explicitly asks.

**Parent skill:** `kart-racer/kr-app-expert` — use that for scheduled reviews, coding improvements, and daily gates. Load **both** skills for weekly audits.

**Not the React Native Hermes JS engine** — this skill is for the Nous Hermes Agent reviewing the KartRacer codebase.

## When to use

- Weekly review (with `kart-racer/kr-app-expert`)
...
4. **Write report:** `docs/reviews/MOBILE_REVIEW_YYYY-MM-DD.md` or contribute to `KR_REVIEW_YYYY-MM-DD.md` when run with rr-app-expert
5. **Do not** commit, push, or fix code — hand off to Cursor

## Step 1 — Preflight gates

From repo root (`C:\KartRacer` or equivalent):
...
```powershell
$env:API_URL="http://localhost:3001"; node scripts/health-check.mjs
```
...
Path: `docs/reviews/MOBILE_REVIEW_YYYY-MM-DD.md` (or section inside `KR_REVIEW_YYYY-MM-DD.md`)
