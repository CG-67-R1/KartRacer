...
const ANDROID_OUT = path.join(ROOT, 'android-app', 'src', 'packs', 'bundled');
...
for (const packId of active.packs) {
  ...
  for (const rel of slices) {
    const src = path.join(srcRoot, rel);
    if (!fs.existsSync(src)) continue;
    copyJson(src, path.join(ANDROID_OUT, packId, rel));
  }
}
fs.mkdirSync(ANDROID_OUT, { recursive: true });
fs.writeFileSync(path.join(ANDROID_OUT, 'active.json'), JSON.stringify(active, null, 2) + '\n');
fs.writeFileSync(
  path.join(ANDROID_OUT, 'registry.json'),
  JSON.stringify(
    {
      version: registry.version,
      nodes: registry.nodes.filter((n) => active.packs.includes(n.packId)),
...
console.log(`sync-app-packs: bundled [${active.packs.join(', ')}] → app + android-app packs/bundled`);
