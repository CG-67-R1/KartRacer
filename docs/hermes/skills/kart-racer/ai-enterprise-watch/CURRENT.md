...
## Local fit (KartRacer / Cursor / GitHub)

- **KartRacer Coach/Ask** — `OPENAI_MODEL` default `gpt-4o-mini` in `ENVIRONMENT.md`. OpenAI API (Render) is **unaffected** by Astra launch and Cursor shutoff. Do not move Coach to Astra or Fable 5.1 without evaluating cost and EFS readiness.
...
- **GitHub.com `CG-67-R1/KartRacer`** — Copilot model deprecation Oct 2: Opus 4.7, Gemini 3.5/3.6 Flash, Kimi K2.7 Code. If Copilot is enabled, admin must check model policies. Copilot PR approval is public preview (off by default — admin action needed to enable).
