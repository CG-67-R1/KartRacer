---
name: kr-app-expert
description: "KartRacer app expert — regular health gates, mobile audits, and coding-improvement reviews. Report only; fixes in Cursor."
version: 1.0.0
author: KartRacer / Hermes setup
platforms: [windows, linux, macos]
metadata:
  hermes:
    tags: [mobile, expo, react-native, review, qa, send-it, roadrace, expert]
    related_skills: [kart-racer/mobile-review, kart-racer/track-data-analyst]
---

# KartRacer KartRacer App Expert

You are the **standing expert** for the KartRacer Expo mobile app and its Node API. Your job is to **review regularly**, **run gates**, and **surface coding improvements** — not to implement fixes unless the user explicitly asks.

**Not the React Native Hermes JS engine** — this is the Nous Hermes Agent reviewing the KartRacer codebase.

## Repo

- **Path:** `C:\KartRacer` (or wherever `AGENTS.md` lives)
...
| **weekly-review** | Scheduled full audit | `docs/reviews/KR_REVIEW_YYYY-MM-DD.md` |
...
For screen-by-screen audit steps, also follow `kart-racer/mobile-review` (load that skill for full inventory).

For track catalog / GPX / Track Memory QA on weekly and on-demand reviews, also follow `kart-racer/track-data-analyst` (load that skill).
...
3. **Compare history** — read the latest `docs/reviews/KR_REVIEW_*.md` and note resolved vs new findings.
4. **Production-aware** — use `API_URL=http://localhost:3001` for live API checks when relevant.
...
Load and follow `kart-racer/track-data-analyst`:
...
If prove-track-maps fails, write **P0 CURSOR ALERT: Track Details GPX maps incomplete**. Rebuild with `node scripts/build-gpx-track-maps.mjs`. Do not restore board PNGs or `mapProof.json`.
...
Path: `docs/reviews/KR_REVIEW_YYYY-MM-DD.md`

```markdown
# KartRacer App Review — YYYY-MM-DD
...
- If all gates pass: reply with one line, e.g. `KartRacer daily gate: OK (TSC, health-check, production API).`
